        ...
        for (int h = 0; h < mapHeight; h++)
        {
            for(int w = 0; w < mapWidth; w++)
            {
				...
				if (result < -2)
                    ...
                    Instantiate(buildings[1], pos, Quaternion.Euler(new Vector3(-90, angles[angleChoisi], 0)));
                else if (result < 4)
                    Instantiate(buildings[2], pos, Quaternion.Euler(new Vector3(-90, angles[angleChoisi], 0)));
                else if (result < 6)
                    Instantiate(buildings[3], pos, Quaternion.Euler(new Vector3(-90, angles[angleChoisi], 0)));
                else if (result < 7)
                    Instantiate(buildings[4], pos, Quaternion.Euler(new Vector3(-90, angles[angleChoisi], 0)));
                else if (result < 10)
